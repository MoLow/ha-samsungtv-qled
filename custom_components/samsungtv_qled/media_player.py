...
# from .samsungtvctl.samsungtvws import SamsungTVWS
from samsungtvws import SamsungTVWS
from .samsungtvctl.samsungtvupnp import SamsungTVUpnp
from .samsungtvctl.smartthings import SmartthingsTV
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_EXPAND_SOURCES, default=True): cv.boolean,
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if ip_addr not in known_devices:
        ...
        add_entities([SamsungTVDevice(host, port, name, timeout, mac, uuid, update_method, update_custom_ping_url, source_list, app_list, api_key, device_id, expand_sources)])
    ...
    def source_list(self):
        ...
        if self._api_key is not None:
            if self._expand_sources:
                self._source_list = [self._smarttv._source_list[i] for i in range(len(self._smarttv._source_list)) if i%2]
            else:
    ...
    async def async_select_source(self, source):
        ...
        if source in self._source_list:
            if self._api_key is None:
                ...
                await self.hass.async_add_job(self._smarttv.select_source, source)
        elif source in self._app_list:
            source_key = self._app_list[ source ]
            await self.hass.async_add_job(self.send_command, source_key, "run_app")
        else:
